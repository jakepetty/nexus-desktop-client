...
#include "NotificationDialog/notificationdialog.h"
...
public slots:
    void onConversionComplete( QByteArray fileData, QString players ) {
        ...
        if ( check["code"].toInt() == 200 ) {
            ...
            data.append( "--" + bound + "\r\n" );
            data.append( "Content-Disposition: form-data; name=\"data[Report][season]\"\r\n\r\n" );
            QMap<QString, QString> fileinfo = STO->patchInfo( Acc->User["install_directory"].toString() + DS + ".patch" + DS );
            data.append( fileinfo["PatchBranchName"] + "\r\n" );
            data.append( "--" + bound + "\r\n" );
        ...
        if ( Acc->DesktopSetting["allow_game_news_notifications"].toBool()) {
            ...
            if ( !data.isEmpty()) {
                if ( settings->value( "last_notification", NULL ).toString() != data["title"].toString()) {
                    ...
                    d->setTitle( data["title"].toString() );
                    d->setBody( data["text"].toString() );
                    d->setURL( data["link"].toString() );
                    d->setAuthor( data["author"].toString() );
                    d->show();
                    settings->setValue( "last_notification", data["title"].toString());
                    settings->sync();
                }
            }
        }
